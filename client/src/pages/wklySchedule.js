...
function WklySchedule() {
  ...
  const [weekMsg, setWeeOfMessage] = useState(
    "Lesson Schedule for the week of " + weekOf.format("dddd, MMMM Do")
  );
  ...
  function checkIfavailable(id) {
    ...
    const lessonBooked = lessons.find((lesson) => lesson.timeSlot === ts);
    if (id.substr(0, 2) === "Sa") console.log("Test..." + id + " " + lessons    );
    availability = "Available";
    if (lessonBooked)
      availability =
        lessonBooked.rider.firstName + " " + lessonBooked.rider.lastName;
  ...
  function makeSchedule(index) {
    return weekdays.map((day) => {
      let time = timeOfAppointment[index]; // 0900
      let id = day.slice(0, 2) + time; // sa0900

      return (
        <td key={id}>
          <a href="/" id={id} onClick={scheduleAlesson}>
            {checkIfavailable(id)}
          </a>
        </td>
      );
      ...
      {data ? (
        <div className="app-container">
          <div className="directional">
            <button type="button" id="backward" onClick={() => goBackward()}>
              Last week
            </button>{" "}
            &nbsp; &nbsp;
            <p>{weekMsg}</p> &nbsp; &nbsp;
            <button type="button" id="forward" onClick={() => goForward()}>
              Next week
            </button>
          </div>
          <LessonForm
